...
namespace lab_3.StepDefinitions
{
    ...
    public class Booking : Client
    {
        ...
        [When(@"send  GET request  ""([^""]*)"" and tell the booking id:(.*)")]
        public void WhenWESendGETRequestToAndTOTellTheBookingId(string url, int id)
        {
        ...
        [When(@"send  POST request ""([^""]*)"" with details ""([^""]*)"" and ""([^""]*)""")]
        public void WhenWESendTHEAPOSTRequestWithTHEDetailsAnd(string url, string name_value, string password_value)
        {
            client = new RestClient(url);
            ...
            var bodyOfRequest = new
            {
                username = name_value,
                password = password_value
            };

            request.AddBody(bodyOfRequest);
            response = client.Execute(request);
        ...
        public void WhenWECreateTHEBookingWithId(int booking_id)
        {
            bookingObject = new BookingObject(booking_id);
        }

        [When(@"send  PUT request ""([^""]*)"" with this booking")]
        public void WhenWESendAPUTRequestWithThisBooking(string url)
        {
            ...
            request.AddJsonBody(bodyOfRequest);
            response = client.Execute(request);
        }

        [When(@"I send a Delete request to ""([^""]*)"" with a booking id:(.*)")]
        public void WhenWESendADeleteRequestWithABookingId(string url, int id)
        {
            client = new RestClient(url.Replace(":id", id.ToString()));
            request = new RestRequest();
        ...
        [When(@"send  POST request ""([^""]*)"" with selected booking details")]
        public void WhenWESendAPOSTRequestWithSelectedBookingDetails(string url)
        {
            client = new RestClient(url);
